...
void lcdInitChars() {
   ...
   * Начертания выверены по эталонной библиотеке LCD_1602_RUS —
   * тот же шрифтовой канон, что в профессиональных блоках
   * русификации (FLprog lcd_v2_18 и подобных): проверенные на
   * реальных дисплеях битмапы, буква «И» с правильной диагональю
   * (сверху-справа вниз-влево, как зеркальная N).               */
  static uint8_t GL[8][8] = {
    {0x07,0x05,0x05,0x05,0x05,0x11,0x11,0x00},  // Л
    {0x11,0x13,0x13,0x15,0x19,0x19,0x11,0x11},  // И
    {0x0E,0x0A,0x0A,0x0A,0x0A,0x1F,0x11,0x00},  // Д
    {0x0F,0x11,0x11,0x0F,0x05,0x11,0x11,0x00},  // Я
    {0x11,0x11,0x11,0x0F,0x01,0x01,0x01,0x00},  // Ч
    {0x15,0x15,0x15,0x15,0x15,0x1F,0x00,0x00},  // Ш
    {0x0A,0x1F,0x10,0x10,0x1E,0x10,0x10,0x1F},  // Ё
...
uint8_t lcdMapChar(uint16_t u) {
  ...
  /* Украинские и расширенные символы — латинскими двойниками
   * (по тому же принципу, что и профессиональные блоки-русификаторы) */
  switch (u) {
    case 0x0404: case 0x0454: return 'E';                // Є є
    ...
    case 0x2013: case 0x2014: return '-';                // – — тире
  }
  uint8_t idx;
