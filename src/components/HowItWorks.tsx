...
const STEPS = [
  {
    n: "01",
    t: "Ресепшен — выдача карты",
    d: "Гость получает карту Mifare на ресепшен — это отдельное рабочее место (третий терминал). Оператор входит в режим регистрации (кнопка или админ-карта) и подносит карту: прошивка выдаёт сквозной номер 1, 2, 3… Повторное касание показывает ФИО и визиты гостя. Защита «одно место за период» строится на уникальном UID карты, а не на номере.",
    tag: "РЕСЕПШЕН · RC522",
  },
  {
    n: "02",
  ...
  },
  {
    ...
    d: "Столовая и ресторан — два контролирующих терминала. Каждый смотрит свой журнал дня И опрашивает другой зал по HTTP (сверка по ключу). Если гость уже отметился в любом из двух залов в этом периоде — красная лампа, звуковой сигнал, вход запрещён. Ресепшен в этой сверке не участвует.",
    tag: "HTTP-сверка",
  },
  {
    n: "04",
  ...
  return (
    <section id="logic" className="relative mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:py-28">
      <SectionHead
        ...
        lead="Сценарий отработан до мелочей: вердикт выносится только после полного цикла проверок, лазерный рубеж страхует от прохода «без касания», а столовая и ресторан сверяются между собой, чтобы правило «одно место за период» работало на оба зала сразу. Ресепшен — третье рабочее место — выдаёт карты и мониторит."
      />
